...
public class TagCloudAnalyzerTests
{
    ...
    {
        var results = new TagCloudAnalyzer(new TagCloudSetting() { MaxCloudSize = 2 }).ComputeTagCloud(new string[] { "hi hi fancy pant pant" });
    ...
    {
        var result = new TagCloudAnalyzer().ComputeTagCloud(original).Shuffle().Select(t => t.Text).ToArray();
        return result;
    }

    private IDictionary<string, int> GetWords(IEnumerable<string> strs)
    {
        return new TagCloudAnalyzer().ComputeTagCloud(strs).ToDictionary(t => !string.IsNullOrEmpty(t.Text) ? t.Text.ToLowerInvariant() : "", e => e.Count);
    }
}